...
class Like(TimeStampedModel):
    ...
    def __str__(self):
        return f"{self.user.first_name} liked {self.article.title}" # type: ignore

    
class Article(TimeStampedModel):
    author = models.ForeignKey(User, on_delete=models.CASCADE, related_name="articles")
    title = models.CharField(verbose_name=_("Title"), max_length=255)
    slug = AutoSlugField(populate_from="title", always_update=True, unique=True), # type: ignore
    description = models.CharField(verbose_name=_("description"), max_length=555)
    ...
    def __str__(self):
        return f"{self.author.first_name}'s article" # type: ignore
    ...
    def view_count(self):
        return self.article_views.count() # type: ignore

    def average_rating(self):
        ratings = self.ratings.all() # type: ignore

        if ratings.count() > 0:
            total_rating = sum(ratings.rating for rating in ratings)
...
class ArticleView(TimeStampedModel):
    ...
    class Meta:
        ...
        verbose_name_plural = _("Ariticle Views")
        unique_together = ("article" , "user", "viewer_ip")

    def __str__(self):
        return f"{self.ariticle.title} viewd by {self.user.firt_name if self.user else 'Anonymous' } from IP {self.viewer_ip}" # type: ignore
